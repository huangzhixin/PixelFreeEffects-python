...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Process images or videos with PixelFree through a Linux Python bridge."
    )
    ...
    parser.add_argument("--narrow", type=float, default=None)
    parser.add_argument("--chin", type=float, default=None)
    parser.add_argument("--v-face", type=float, default=None, dest="v_face")
    ...
    parser.add_argument("--ruddy", type=float, default=None)
    ...
    parser.add_argument("--one-key", type=int, choices=[0, 1, 2, 3, 4], default=None, dest="one_key")
    parser.add_argument("--rotation", type=int, choices=[0, 90, 180, 270], default=0)
    parser.add_argument("--progress-every", type=int, default=30)
    return parser.parse_args()


def rotation_to_enum(rotation: int) -> PFRotationMode:
    mapping = {
...
def apply_params(bridge: PixelFreeBridge, args: argparse.Namespace) -> None:
    if args.filter_name:
    ...
    if args.one_key is not None:
        bridge.set_int_param(PFBeautyFiterType.PFBeautyFiterTypeOneKey, args.one_key)


def process_image(input_path: Path, output_path: Path, args: argparse.Namespace) -> None:
    with PixelFreeBridge(auth_path=str(args.auth), filter_path=str(args.filter_bundle)) as bridge:
...
def process_video(input_path: Path, output_path: Path, args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="beauty_video_") as temp_dir:
        temp_video = Path(temp_dir) / output_path.name
        with PixelFreeBridge(auth_path=str(args.auth), filter_path=str(args.filter_bundle)) as bridge:
            apply_params(bridge, args)
...
def validate_paths(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    if args.auth is None:
        args.auth = str(linux_sdk_dir / "Res" / "pixelfreeAuth.lic")
    if args.filter_bundle is None:
        args.filter_bundle = str(linux_sdk_dir / "Res" / "filter_model.bundle")
