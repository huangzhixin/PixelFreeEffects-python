...
#include "../../SMBeautyEngine_windows/pixelfreeLib/Include/pixelFree_c.hpp"
#include "../../SMBeautyEngine_windows/include/opengl.h"
#include <glad/glad.h>
...
extern "C" __declspec(dllexport) const char* PFPy_GetLastError() {
    return g_last_error.c_str();
}

extern "C" __declspec(dllexport) void* PFPy_CreateEngine(const char* auth_path, const char* filter_path) {
    SetLastErrorMessage("");
...
extern "C" __declspec(dllexport) int PFPy_SetFloatParam(void* engine, int key, float value) {
    SetLastErrorMessage("");
    ...
    glfwMakeContextCurrent(bridge->window);
    PF_pixelFreeSetBeautyFiterParam(bridge->handle, key, &value);
    return 1;
}

extern "C" __declspec(dllexport) int PFPy_SetStringParam(void* engine, int key, const char* value) {
    SetLastErrorMessage("");
    auto* bridge = reinterpret_cast<PixelFreeBridge*>(engine);
    if (bridge == nullptr || bridge->handle == nullptr) {
        SetLastErrorMessage("Engine is null");
    ...
    PF_pixelFreeSetBeautyFiterParam(
        bridge->handle,
        key,
...
extern "C" __declspec(dllexport) int PFPy_ProcessImageRGBA(
    void* engine,
    uint8_t* rgba_data,
    int width,
    int height,
...
extern "C" __declspec(dllexport) void PFPy_DestroyEngine(void* engine) {
    auto* bridge = reinterpret_cast<PixelFreeBridge*>(engine);
    if (bridge == nullptr) {
        return;
    }
